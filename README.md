...
在页头打开「我的物料」。首次没有作品时，点击「新建物料」进入编辑器；之后默认继续上次编辑的作品。每张卡片展示贺图缩略图与修改时间，按最近修改排序。点击卡片继续编辑，点击更多操作可复制、重命名或删除。名称用于管理作品，不改变图片中的姓名；支持 1～60 个字素。删除需要确认，删除最后一件作品后保持空列表。
...
保存队列捕获作品 ID 与内容快照，退出前完成保存；已离开的上传、解码与渲染任务不会修改下一件作品。缩略图由同一 Canvas 渲染器生成 320 × 400 PNG，异步保存时核对内容版本，失败不影响作品保存。删除后的迟到保存不能重新创建作品；缩略图更新和打开作品不会改变内容修改时间。
...
数据库升级后旧版应用不再能打开该数据库版本；不要通过清理站点数据来尝试降级。原单份草稿作为一次迁移备份保留在浏览器中。应用没有项目文件导入导出或云备份功能。
...
已发布模板的 `id` 应保持稳定。若要改变已有主题的布局含义，请用新的 `id` 并保留旧主题，避免改变用户恢复的作品。模板 `version` 是元信息，作品记录 schema 与项目状态版本目前均为 2；旧单草稿 schema 1 通过上述显式迁移保留。未知版本或缺失主题不能静默替换成默认作品。
