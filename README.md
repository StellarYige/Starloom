...
1. 在「我的物料」点击「制作电子小卡」；「制作生日应援」创建原有生日应援作品。空列表展示两种物料的真实成品示例；已有作品时仍默认继续上次编辑。
...
5. 选择配色与装饰：可点击颜色方块打开可视化选色器，也可输入 HEX 色值；与生日编辑器共用控件，支持撤销重做。点击「导出电子小卡 PNG」。看到「已保存到本机」后再关闭浏览器，下次用相同浏览器资料和站点继续编辑。
...
| 部署方式                        | 设置                                                              |
| ------------------------------- | ----------------------------------------------------------------- |
| GitHub Pages                    | Actions 执行 `npm run build:pages`，发布检查过的 `dist/` artifact |
...
本仓库的 Pages 构建显式使用 `/Starloom/`。工作流见 [pages.yml](.github/workflows/pages.yml)：PR 只执行格式检查、单元测试、完整 Chromium 回归、独立 WebKit 核心测试、生产构建和子路径验证；`main` 的检查全部通过后，使用 `upload-pages-artifact` / `deploy-pages` 发布同一份 `dist/`，不创建长期发布分支，也不上传测试资料。手动运行仅在 `main` 部署。仓库 Settings → Pages 的 Source 需为 **GitHub Actions**。
...
测试用独立 4175 端口访问 `/Starloom/`，验证图片、字体和实际 PNG 下载。Pages 与本机开发站点来源不同，浏览器中的本机作品不会自动出现在在线站点。
...
0.3.1 沿用上述数据结构。读取旧作品时，为当前会话取得独立照片字节副本，避免 WebKit 后续保存替换磁盘文件后影响当前照片的读取；保留原照片字节、作品 ID、版本与时间，读取本身不改写记录。
...
存储空间不足时，应用显示保存失败，仍允许编辑和导出；新建或复制失败不改变其他作品。本地存储完全不可用时，可选择「临时制作并导出」，此时没有保存能力。临时作品离开前会询问「放弃临时作品？」；选择「继续制作」保留当前编辑和进行中的上传，选择「放弃并离开」后允许返回列表，无需等待不可能成功的保存。正常作品仍需保存成功才能离开。损坏或不兼容的作品不会被示例内容自动覆盖，其他可读取的作品仍可管理。本轮不支持多标签页协同编辑，请避免在多个标签页同时修改同一作品。
...
public/
  assets/        随项目发布的示例照片
  examples/      经界面实际导出的授权示例、缩略图与校验清单
...
以下示例由 0.3.1 的实际下载按钮生成，点击缩略图查看原尺寸 PNG。使用已有授权示例照片，姓名与日期均为虚构；两种小卡示例不显示日期。

| 生日来信贺图                                                                                 | 上下照片条                                                                         | 写真拼贴                                                                                   |
| -------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| [![生日来信贺图](public/examples/birthday-poster.webp)](public/examples/birthday-poster.png) | [![上下照片条](public/examples/photo-strip.webp)](public/examples/photo-strip.png) | [![写真拼贴](public/examples/portrait-collage.webp)](public/examples/portrait-collage.png) |

[配套头像 PNG](public/examples/birthday-avatar.png) · [尺寸与 SHA-256](public/examples/manifest.json) · [素材授权](public/licenses/ASSETS.md)。启动 `npm run dev` 后，在另一终端执行 `npm run examples` 可重新生成这四张公开示例；只读取仓库内授权素材，使用全新浏览器会话，WebP 缩略图由下载的 PNG 缩小。它们是明确收录的产品素材，日常测试下载仍只留在 `.qa/`。
...
`desktop` / `mobile` 使用 Chromium 引擎运行全部旧回归与新增测试；其中 CDP 触屏手势和完整进程 PID 重启验证仅属于 Chromium。`webkit-desktop` / `webkit-mobile` **实际启动 WebKit**，独立运行 `core-workflow.spec.ts` 与 `experience.spec.ts`，验证制作入口、成品示例、临时退出确认、正常保存失败保护、选色器、两种物料的制作、照片及裁切恢复、默认继续上次作品、字体与实际导出。

只运行 WebKit：

```sh
npm run test:e2e -- --project=webkit-desktop --project=webkit-mobile
```

生产验证从 `dist/` 启动 4175 端口，在 `/Starloom/` 覆盖上述共享流程、两布局带 / 不带日期的真实下载，以及 Chromium 浏览器重启恢复；报告写入 `.qa/production-results.json`。所有测试浏览器资料、上传测试图、下载 PNG 和截图都在被 Git 忽略的 `.qa/`，不上传。

Windows 的 Chromium 项目默认使用本机 Microsoft Edge；Linux / CI 默认使用 Playwright 自带 Chromium。WebKit 项目不继承 `PLAYWRIGHT_CHANNEL`。测试均使用独立无头会话，不读取个人浏览器资料。也可为 Chromium 项目指定已安装的 Chrome：
...
线上验收时设置 `STARLOOM_ONLINE_URL` 为部署返回且确认可访问的 `/Starloom/` 入口，再执行 `npm run test:online`；该配置不开本机服务器，使用两种引擎执行 `@core` 流程。可按本机网络需要设置 `STARLOOM_TEST_PROXY`，不修改应用网络行为。
