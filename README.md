...
- 可导出 `.starloom` 本地工程文件，包含照片和全部编辑内容；在「我的物料」导入为独立新作品，不覆盖原作品。
- 模板装饰文案支持修改、隐藏和恢复默认，沿用模板位置与字体，支持撤销重做和自动保存。
...
编辑器顶部的「导出工程」下载当前编辑内容，包括一张或两张照片的实际字节、照片信息、各主题和布局的全部裁切记忆、姓名、日期、短句或祝福、配色、装饰开关与装饰文案。即使修改尚未自动保存，或文字过长暂时不能导出 PNG，仍可备份工程。文件不包含撤销历史；PNG 仍是成品图片，不能恢复编辑内容。

在「我的物料」点击「导入工程」，选择 `.starloom` 文件。校验成功后创建名称带「（导入）」的新作品，并直接打开编辑器；每次导入均使用新的作品和照片 ID。导入失败不创建半成品，也不更改原作品或最近编辑指针，可重新选择同一文件重试。全部读写在本机完成，可手动传递文件到其他浏览器或设备。

工程格式当前为 `starloom-project` 版本 1，使用 JSON 和内嵌 Base64 照片，文件上限为 100 MiB（界面显示 100 MB）。导入检查结构、照片引用、裁切范围、装饰文案、SHA-256、实际照片解码与尺寸；未知工程版本、作品版本、模板或模板版本不匹配会明确拒绝，不猜测布局。完整性校验需要 HTTPS 或本机 `localhost` / `127.0.0.1` 的安全上下文。没有云备份或自动跨设备同步。

生日作品在「写祝福」展开「模板装饰文案」，小卡在设置区下方展开同名设置。每条可输入文字、切换「显示」或「恢复默认」，分别记住每个模板的修改。每条最多 80 字素；文字超出模板区域会提示并阻止 PNG 导出，可以缩短或隐藏。位置、字体和布局由模板固定，不提供自由画布。
...
缩略图由同一 Canvas 渲染器生成：生日贺图为 320 × 400，小卡为 300 × 400。异步保存时核对内容版本，失败不影响作品保存；只换第二张照片也会更新版本。删除后的迟到保存不能重新创建作品；缩略图更新和打开作品不会改变内容修改时间。编辑期间，内容保存和缩略图完成的回调不再读取整个作品列表；返回「我的物料」时刷新，列表可见期间的迟到缩略图仍会更新。
...
0.3 继续使用数据库版本 2，原 0.2 作品的 schema 和项目版本仍为 2，不做批量重写。仅新小卡使用 schema / 项目版本 3，新增第二张照片、可选日期与双槽位布局记忆。缺图、损坏、未知版本或缺失布局会报错并保留原记录，不能回退成示例或生日作品。原单份草稿作为一次迁移备份保留在浏览器中；不要通过清理站点数据尝试降级。本轮沿用数据库和作品版本，仅增加可选的装饰文案字段；旧作品缺少该字段时保持原模板效果。
...
可用图层：`photo`、`text`、`rect`、`line`、`sparkle`。文字 `content` 可以绑定 `name`、`birthday`、`wish`，或写成 `{ id: 'poster-heading', literal: '默认装饰文案' }`。固定文案 ID 在同一模板的所有成品布局内须唯一且稳定，使用小写字母和连字符；编辑值保存在 `templateTexts[templateId][id] = { text, hidden }`，省略时显示默认文案。改变布局或默认文案时须增加模板 `version`，以便工程文件检测兼容性。颜色使用 `paper`、`ink`、`muted`、`accent`、`soft`、`white`、`onAccent`；渲染器根据应援色与模板明暗模式派生颜色。深色模式的 `ink`、`muted` 为浅色，色块文字使用 `onAccent`；不要把低对比的装饰色直接用于重要文字。
